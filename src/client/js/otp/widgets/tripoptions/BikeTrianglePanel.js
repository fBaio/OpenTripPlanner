...
otp.widgets.BikeTrianglePanel = new otp.Class({
    ...
    //for speed
    quickName   : _tr("Open Areas"),
    //TRANSLATORS: Optimization for bicycle shown in bike triangle. Optimized
    //for flat terrain
    flatName    : _tr("Green Areas"),
    //TRANSLATORS: Optimization for bicycle shown in bike triangle. Optimized
    //for bike friendly infrastructure. Cycle roads etc...
    safeName    : _tr("Historical Areas"),
    ...
    render : function() {
        ...
        this.triangle = canvas.path(["M",margin+tri_side/2,margin,"L",margin+tri_side,height-margin,"L",margin,height-margin,"z"]);
		this.trianglecolor = "rgb("+ (255*this.quickFactor) + "," +(255*this.flatFactor) + "," +  (255*this.safeFactor)+")";

		
        this.triangle.attr({fill:this.trianglecolor, stroke:"none"});
        
        var labelSize = "18px";

        var safeFill = "#ff0000"; 
        var safeFill2 = "#ffffff"; 
        //TRANSLATORS: First letter of Bike Friendly
        var safeSym  = _tr("R"); 

        var flatFill = "#00c200"; 
        var flatFill2 = "#ffffff"; 
        //TRANSLATORS: First letter of Flat
        var flatSym  = _tr("G"); //locale.bikeTriangle.flatSym;
        
        var quickFill = "#0000ff";
        var quickFill2 = "#ffffff";
        //TRANSLATORS: First letter of Quick
        var quickSym  = _tr("B"); //locale.bikeTriangle.quickSym;
        ...
        move = function (dx, dy) {
			...
			// codice baio
			var b = Math.round(255*time) < 0 ? 0 : Math.round(255*time);
			var g = Math.round(255*topo) < 0 ? 0 : Math.round(255*topo);
			var r = Math.round(255*safety) < 0 ? 0 : Math.round(255*safety);
			this_.trianglecolor = "rgb("+ r + "," + g + "," +  b +")";
			
			this_.triangle.attr({fill:this_.trianglecolor, stroke:"none"});
